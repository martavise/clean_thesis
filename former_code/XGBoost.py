...
def RepeatedStratifiedAutoML(X, y, grouplabels, n_repeats=3, k=5, pca_threshold=100, estimator='xgboost'):
    Acc = []
    AUC = []
...
def RepeatedStratifiedXGB(X, y, grouplabels, n_repeats=3, k=5, pca_threshold=30, search=False):
    Acc = []
    AUC = []
...
for Name in TDAFeatureNames:
    ...
    for i in range(Feature_In.shape[1]):
        ...
        Summary = {f'Accuracy_{Name}_{i}': mean(Acc), f'ROC_AUC_{Name}_{i}': mean(AUC)}
        SumBoard_Flaml.update(Summary)
...
for Name in TDAFeatureNames:
    ...
    Summary = {f'Accuracy_{Name}_Concat': mean(Acc), f'ROC_AUC_{Name}_Concat': mean(AUC)}
    SumBoard_Flaml.update(Summary)
...
for Name in TDAFeatureNames:
    ...
    Summary = {f'Accuracy_{Name}_FCConcat': mean(Acc), f'ROC_AUC_{Name}_FCConcat': mean(AUC)}
    SumBoard_Flaml.update(Summary)


# saves the results locally
with open(os.path.join(MainDir, 'originalXGboostResultsFLAML.pkl'), 'wb') as f:
...
with open(os.path.join(MainDir, 'originalXGboostResults.pkl'), 'rb') as f:
...
def run_randomized_xgb(
    ...
    for Name in FCFeatureNames:
        ...
        Summary = {
            f'Accuracy_{Name}': mean(Acc),
            f'ROC_AUC_{Name}': mean(AUC)
        }
        SumBoard_Randomized.update(Summary)
    ...
    for Name in TDAFeatureNames:
        ...
        for i in range(Feature_In.shape[1]):
            ...
            Summary = {
                f'Accuracy_{Name}_{i}': mean(Acc),
                f'ROC_AUC_{Name}_{i}': mean(AUC)
            }
            SumBoard_Randomized.update(Summary)
    ...
    for Name in TDAFeatureNames:
        ...
        Summary = {
            f'Accuracy_{Name}_Concat': mean(Acc),
            f'ROC_AUC_{Name}_Concat': mean(AUC)
        }
        SumBoard_Randomized.update(Summary)
    ...
    for Name in TDAFeatureNames:
        ...
        Summary = {
            f'Accuracy_{Name}_FCConcat': mean(Acc),
            f'ROC_AUC_{Name}_FCConcat': mean(AUC)
        }
        SumBoard_Randomized.update(Summary)

    # Save results
    out_file = os.path.join(MainDir, 'originalXGboostResultsRandom.pkl')
    with open(out_file, 'wb') as f:
        pickle.dump(SumBoard_Randomized, f)

    print("Saved Randomized Search results to 'originalXGboostResultsRandom.pkl'")
